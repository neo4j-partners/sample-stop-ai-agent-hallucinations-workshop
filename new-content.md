...
- The notebook must end by calling `apply_lab4_fixtures()` from `workshop.graph_setup`. That seeds the fixture Hotel, the `max_guests` Rule, and three constraints that Labs 4 and 5 depend on. Without it, Lab 4 opens onto an unprepared graph. Section 7 calls it and then asserts `readiness_problems()` is empty, so a partial seed fails the notebook instead of passing quietly.
...
**AWS technologies.** Bedrock for the memory embeddings only, through Titan Text Embeddings V2 at 1024 dimensions, writing to the memory graph's own two vector indexes rather than to Lab 1's Nova chunk index. No model reasons here: the lab writes its memory explicitly, so nothing needs an LLM to extract. AgentCore Memory is the managed contrast.
...
- ~~**The flow diagram.**~~ Closed. `images/six-lab-path.svg` is drawn and the root README renders it at line 35. The old `images/why-ai-agents-fail-six-demos-progressive-flow.png` has been deleted, since nothing referenced it and its filename used retired vocabulary.
...
- ~~**Documentation that the rebuild left behind.**~~ Closed by the post-rebuild quality review; the findings and their outcomes are at the bottom of `new-content-plan.md`. Every item this bullet listed is now fixed: `workshop-delivery/README.md` is a six-lab facilitator guide, every Python badge reads 3.12+, `04-grounded-write/README.md` names Sonnet 5, and the root README's build-status note matches Phase 8's live run. One prose line still states a lower floor than the tree installs, `05-agentcore-deploy/README.md:114`, which gives 3.11+ as the floor for `workshop_cleanup.py` because `enum.StrEnum` needs it.
